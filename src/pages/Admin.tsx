...
import { Plus, Edit2, Trash2, Check, RefreshCw, UploadCloud, Sliders, CheckCircle, Tag, ShoppingBag } from 'lucide-react';
...
const PDF_CATALOG_SNEAKERS: Shoe[] = [
  ...
  },
  {
    ...
    description: 'Inspiradas en el ritmo de la ciudad, las Pulse Roam combinan una estética urbana con durabilidad deportiva. Amortiguación Air de alta respuesta y detalles en amarillo neón.',
    sizes: ['40', '41', '42', '43', '44'],
    colors: [{ name: 'Beige', hex: '#f5f5dc' }, { name: 'Amarillo', hex: '#eab308' }]
  },
  {
    ...
    description: 'El clásico atemporal del streetwear. El bloque de color blanco y negro retro es versátil, cómodo y perfecto para el skateboarding o el estilo casual diario.',
  ...
  },
  {
    ...
    description: 'Con el inconfundible grafiti de AIR en los costados, estas Uptempo rinden homenaje al baloncesto de los 90. Máxima amortiguación Air-Sole visible en colorway blanco y azul cobalto.',
    sizes: ['40', '41', '42', '43', '44', '45', '46'],
    colors: [{ name: 'Blanco', hex: '#ffffff' }, { name: 'Cobalto', hex: '#1d4ed8' }]
  },
  ...
  },
  {
    ...
    description: 'Zapatilla de running ligera para entrenamientos diarios. Amortiguación Lightmotion extremadamente suave y un diseño superior de malla transpirable de color verde neón.',
    sizes: ['38', '39', '40', '41', '42', '43', '44'],
    colors: [{ name: 'Neón', hex: '#84cc16' }, { name: 'Negro', hex: '#000000' }]
  },
  {
    id: 'nb-9060-sea-salt',
  ...
  },
  {
    ...
    price: 102, // S/ 390
    imageUrl: 'https://images.unsplash.com/photo-1515955656352-a1fa3ffcd111?q=80&w=800&auto=format&fit=crop',
    description: 'Diseñadas para el estilo dinámico de juego de Luka Dončić. Soporte lateral optimizado, amortiguación reactiva Formula 23 y una estética negra y verde volt vibrante.',
    sizes: ['41', '42', '43', '44', '45'],
    colors: [{ name: 'Negro', hex: '#000000' }, { name: 'Volt', hex: '#a3e635' }]
  }
];
...
export default function Admin() {
  ...
  const handleEditClick = (shoe: Shoe) => {
    ...
    const colorStr = shoe.colors 
      ? shoe.colors.map(c => `${window.btoa(encodeURIComponent(c.name)) ? c.name : c.name}:${c.hex}`).join(', ')
      : '';
  ...
  const handleDeleteClick = async (id: string) => {
    if (!window.confirm('¿Estás seguro de que deseas eliminar este sneaker del catálogo?')) return;
    try {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    const colorArray = formData.colors.split(',').map(c => {
    ...
    const shoePayload: Shoe = {
      ...
      imageUrl: formData.imageUrl || 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?q=80&w=800&auto=format&fit=crop',
      gallery: [formData.imageUrl || 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?q=80&w=800&auto=format&fit=crop'],
    ...
    try {
      ...
      showToast(isEditing ? 'Sneaker actualizado correctamente' : 'Nuevo sneaker añadido al catálogo', 'success');
      handleResetForm();
      fetchProducts();
    } catch (error: any) {
      console.error(error);
  ...
  const handleSeedCatalog = async () => {
    if (!window.confirm('¿Deseas agregar todos los modelos destacados de los nuevos catálogos de Nike, Adidas, Jordan y Puma a tu base de datos?')) return;
    setIsSaving(true);
    try {
      let count = 0;
      ...
      showToast(`¡Se han importado ${count} sneakers icónicos de los catálogos en PDF con éxito!`, 'success');
      fetchProducts();
    } catch (error: any) {
      console.error(error);
      const errMsg = error?.message || (typeof error === 'string' ? error : 'Error desconocido');
    ...
    <div className="min-h-screen bg-[#F9F9F9] dark:bg-slate-950 py-12 text-slate-900 dark:text-slate-100 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <span className="text-blue-600 dark:text-blue-400 font-black text-xs uppercase tracking-[0.2em] mb-2 block">Base de Datos Firestore</span>
            <h1 className="text-4xl font-black tracking-tighter uppercase italic">PANEL DE ADMINISTRACIÓN</h1>
            <p className="text-gray-500 dark:text-slate-400 text-sm max-w-xl mt-1">
              Agrega, edita y gestiona el catálogo de zapatillas de tu tienda directamente en la nube.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-[32px] p-6 shadow-sm h-fit">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 dark:text-slate-500 mb-1.5">Nombre del Sneaker</label>
                <input 
                  type="text" 
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-black uppercase tracking-wider text-gray-400 dark:text-slate-500 mb-1.5">Marca</label>
                  <input 
                    type="text" 
                    required
                ...
                <div>
                  <label className="block text-xs font-black uppercase tracking-wider text-gray-400 dark:text-slate-500 mb-1.5">Categoría</label>
                  <select 
                    className="w-full bg-gray-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none text-slate-800 dark:text-slate-100"
                    value={formData.type || 'Lifestyle'}
                    onChange={(e) => setFormData({ ...formData, type: e.target.value as Shoe['type'] })}
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-black uppercase tracking-wider text-gray-400 dark:text-slate-500 mb-1.5">Precio (USD)</label>
                  <input 
                    type="number" 
                    required
                    min="1"
                ...
                <div className="flex items-end pb-3 pl-2">
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    ...
                    />
                    <span className="text-xs font-black uppercase tracking-wider text-gray-400 dark:text-slate-500">¿Es Novedad?</span>
                  </label>
                </div>
              </div>

              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 dark:text-slate-500 mb-1.5">URL de Imagen</label>
                <input 
                  type="url" 
                  placeholder="https://..."
                  className="w-full bg-gray-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none text-slate-800 dark:text-slate-100"
              ...
              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 dark:text-slate-500 mb-1.5">Descripción</label>
                <textarea 
                  rows={3}
              ...
              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 dark:text-slate-500 mb-1.5">Tallas disponibles (Separadas por comas)</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 dark:text-slate-500 mb-1.5">Colores (Nombre:Hex, separados por comas)</label>
                <input 
                  type="text" 
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-[32px] p-6 shadow-sm overflow-hidden flex flex-col">
            ...
            {isLoading ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center py-24 text-center space-y-4">
                ...
                <div>
                  <p className="font-bold text-lg text-slate-800 dark:text-slate-100">Catálogo vacío</p>
                  <p className="text-sm text-gray-400 dark:text-slate-500 max-w-sm mt-1">
                    No hay ningún producto registrado en Firestore. Pulsa "Importar Catálogos PDF" o añade uno manualmente.
                  </p>
                </div>
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] font-black uppercase tracking-wider text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 px-2 py-0.5 rounded-full">{shoe.brand}</span>
                        <span className="text-[10px] font-black uppercase tracking-wider text-gray-400 dark:text-slate-500 bg-gray-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">{shoe.type}</span>
                        {shoe.isNew && <span className="text-[10px] font-black uppercase tracking-wider text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/20 px-2 py-0.5 rounded-full">Nuevo</span>}
                      </div>
                      <h3 className="font-bold text-sm text-slate-800 dark:text-slate-100 truncate">{shoe.name}</h3>
                      <p className="text-xs text-gray-400 dark:text-slate-500 line-clamp-1 mt-0.5 mb-2">{shoe.description || 'Sin descripción'}</p>
                      
                      <div className="flex items-center justify-between">
                        <p className="font-black text-sm text-blue-600 dark:text-blue-400">${shoe.price}</p>
            ...
            <div className="mt-6 p-4 bg-blue-50/50 dark:bg-blue-950/10 rounded-2xl border border-blue-100/30 dark:border-blue-900/10 text-xs text-slate-500 dark:text-slate-400 space-y-2">
              ...
              <p>
                Este panel conecta directamente con la base de datos <strong>Cloud Firestore</strong> provista por Firebase. Cada zapatilla agregada o editada aquí se sincronizará automáticamente para todos los usuarios que visiten la tienda, incluyendo el catálogo, las secciones de Hombre/Mujer y la búsqueda por imágenes.
              </p>
            </div>
          </div>
