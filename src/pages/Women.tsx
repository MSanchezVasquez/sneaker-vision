...
export default function Women() {
  ...
  const availableBrands = useMemo(() => ['On', 'Vans', 'Reebok', 'Nike'], []);
  const availableTypes = useMemo(() => Array.from(new Set(products.map(s => s.type))), [products]);

  // Filter for women (mock)
  const filteredShoes = useMemo(() => {
    return products.filter(s => {
      const isWomenBrand = availableBrands.includes(s.brand);
      ...
      return isWomenBrand && brandMatch && typeMatch && priceMatch;
    });
        ...
        <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-5xl font-black tracking-tighter mb-4 uppercase italic text-slate-900 dark:text-white">
              Mujer {typeFilter && <span className="text-blue-600 dark:text-blue-400">/ {typeFilter}</span>}
            </h1>
            <p className="text-gray-500 dark:text-slate-400 max-w-2xl">Elegancia, confort y tecnología en cada paso. Descubre nuestra colección femenina.</p>
          </div>
          <button 
